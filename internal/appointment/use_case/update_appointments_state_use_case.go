...
func (u *UpdateAppointmentsStateUseCase) AddAppointment(ctx context.Context, app appointment.AppointmentAggregate) {
	if err := u.trackingService.AddAppointment(ctx, app); err != nil {
		u.log.Error(ctx, "failed to add appointment", sl.Err(err))
	}
}

func (u *UpdateAppointmentsStateUseCase) RemoveAppointment(ctx context.Context, app appointment.AppointmentAggregate) {
